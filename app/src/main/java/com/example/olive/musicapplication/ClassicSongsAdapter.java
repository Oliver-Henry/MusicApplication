...
class ClassicSongsAdapter extends RecyclerView.Adapter<ClassicSongsAdapter.MyViewHolder> {
    Context applicationContext;
    Consumer<MusicModel> consumer;
    List<Result> results;
    int row;

    public ClassicSongsAdapter(Context applicationContext, Consumer<MusicModel> consumer, List<Result> results, int row) {
    ...
    @Override
    public ClassicSongsAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(row, parent, false)) ;
    }

    @Override
    public void onBindViewHolder(ClassicSongsAdapter.MyViewHolder holder, int position) {
